...
public class Money implements Comparable {
	...
	public String toString() {
		...
		double dab =Math.round((double) amount)/100.0;
		df.setMinimumFractionDigits(2);
		df.setMaximumFractionDigits(2);
		return (((dab==0) ? "0,00" : df.format(dab))).replace(".",",") + " " + currency.getName();
	}
	...
	public Integer universalValue() {
		return (int) (currency.getRate() * amount);
	}
	...
	public Money add(Money other) {
		return new Money(currency.valueInThisCurrency(other.amount, other.currency) + amount, currency);
	}
	...
	public int compareTo(Object other) {
		...
		if (!(other instanceof Money)) throw new ClassCastException("Object is not an instance");
		int thisValue = this.universalValue();
		Money mon1 = (Money) other;
		int otherValue = mon1.universalValue();
